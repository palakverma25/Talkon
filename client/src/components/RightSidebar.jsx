...
const RightSidebar = () => {
  ...
  // Get all image URLs from the messages and store them
  useEffect(() => {
    const images = messages
      .filter(msg => msg.image && typeof msg.image === 'string')
      .map(msg => msg.image);

    console.log("Extracted images from messages:", images);
    setMsgImages(images);
  }, [messages]);

  // Render only if a user is selected
  return selectedUser && (
    <div className={`bg-[#8185B2]/10 text-white w-full relative overflow-y-scroll ${selectedUser ? "max-md:hidden" : ""}`}>
      ...
      <div className='px-5 text-xs'>
        <p className='text-white/70 font-semibold'>Media</p>
          ...
          {msgImages.length > 0 ? (
            msgImages.map((url, index) => (
              <div key={index} onClick={() => window.open(url, '_blank')} className='cursor-pointer rounded'>
                <img src={url} alt={`Media-${index}`} className='h-[100px] w-full object-cover rounded-md' />
              </div>
            ))
          ) : (
            <p className='col-span-2 text-center text-sm text-gray-400'>No media available</p>
          )}
        </div>
      </div>
